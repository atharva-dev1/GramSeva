...
export function HomeScreen({ onNavigate, onToast }) {
  ...
  // After voice done → navigate to schemes automatically
  useEffect(() => {
  ...
  const services = [
    {
      ...
      gradient: 'linear-gradient(135deg, rgba(255,107,53,0.6), rgba(255,107,53,0.3))',
      tab: 'schemes',
    },
    {
      emoji: '📈',
      ...
      gradient: 'linear-gradient(135deg, rgba(255,215,0,0.5), rgba(255,150,0,0.3))',
      tab: 'mandi',
    },
    {
      emoji: '🏥',
      ...
      gradient: 'linear-gradient(135deg, rgba(0,212,106,0.5), rgba(0,150,70,0.3))',
      tab: 'health',
    },
    {
      emoji: '⚖️',
      ...
      gradient: 'linear-gradient(135deg, rgba(130,100,255,0.5), rgba(80,60,200,0.3))',
      tab: 'rights',
    },
  ];
  ...
  const handleServiceClick = (tab) => {
    if (tab === 'rights') {
      onToast?.({ type: 'info', title: 'Coming Soon!', message: 'Legal guidance feature launching next update 🚀' });
      return;
    }
    onNavigate(tab);
  };
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <div style={{
                ...
                fontFamily: isHindi ? 'Noto Sans Devanagari, sans-serif' : 'Poppins, sans-serif',
              }}>
                {getTimeGreeting(t)} 🌟
              </div>
            ...
            <div style={{ fontSize: 52, lineHeight: 1, filter: 'drop-shadow(0 0 12px rgba(255,107,53,0.3))' }}>
              🌾
            </div>
          </div>

          {/* Quick stats inside greeting card */}
          <div style={{ display: 'flex', gap: 12, marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,107,53,0.15)' }}>
            {[
              { label: '20 Schemes', emoji: '📋' },
              { label: '15 Crops', emoji: '🌾' },
              { label: '22 Languages', emoji: '🗣️' },
            ].map(s => (
          ...
          <AnimatePresence mode="wait">
            ...
            {(speechState === 'listening' || speechState === 'processing') && (
              <motion.div key="listening" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ textAlign: 'center', maxWidth: 300 }}>
                <div style={{
                  fontSize: 13, color: speechState === 'processing' ? 'var(--accent-secondary)' : 'var(--accent-primary)',
                ...
                {transcript && (
                  <div style={{
                    fontSize: 14, color: 'var(--text-primary)', background: 'var(--bg-elevated)',
                    border: '1px solid rgba(255,107,53,0.2)', borderRadius: 12, padding: '10px 16px', fontStyle: 'italic',
                  }}>
                    "{transcript}"
                  </div>
                )}
            ...
            {speechState === 'done' && showResult && (
              ...
              >
                <div style={{ fontSize: 13, color: 'var(--accent-green)', fontWeight: 600, marginBottom: 8 }}>
                  ✅ Opening matching schemes...
                </div>
                <div style={{
                  fontSize: 14, color: 'var(--text-primary)', background: 'var(--bg-elevated)',
                  border: '1px solid rgba(0,212,106,0.2)', borderRadius: 12, padding: '10px 16px',
                }}>
                  "{transcript}"
                </div>
              </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {services.map((s, i) => (
              <ServiceCard
                ...
                subtitle={s.subtitle}
                gradient={s.gradient}
                onClick={() => handleServiceClick(s.tab)}
                delay={i * 0.07}
              />
            ))}
          </div>
